# tools/patch_py/apply_hotfix_mt5_guard_missing.py
...
def w(p: Path, s: str):
    ...
    print("[HOTFIX] wrote", p)

# --- minimal atomic helpers (local fallback) ---
ATOMIC = r"""
from __future__ import annotations
from pathlib import Path
...
# --- mt5_guard (lean, idempotent outbox + basic ops) ---
GUARD = r"""
from __future__ import annotations
from pathlib import Path
...
def patch_server():
    ...
    print("[HOTFIX] server.py patched (mt5_resilience_router)")

def main():
    # ensure package init
    w(INTEG / "__init__.py", "# integrations pkg")
    # atomic local fallback for guard
    w(INTEG / "_atomic_local.py", ATOMIC)
    # guard + api
    w(INTEG / "mt5_guard.py", GUARD)
    w(APP / "ext_mt5_resilience.py", EXT)
    patch_server()
    print("[HOTFIX] mt5_guard recreated and router wired.")
