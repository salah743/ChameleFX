# tools/patch_py/apply_bundle_O_alpha_feedback.py
...
def patch_server():
    ...
    imp = "from app.api.ext_alpha_feedback import router as alpha_feedback_router"
    inc = "app.include_router(alpha_feedback_router)"
    if imp not in rest:
        rest = imp + "\n" + rest
    if inc not in rest:
        rest += "\n" + inc + "\n"
    (APP/"server.py").write_text(future + rest, encoding="utf-8")
    print("[O] server.py patched (alpha_feedback_router)")

def seed_runtime():
    f = RUN / "regime_sizing.json"
    if not f.exists():
        f.write_text(json.dumps(DEFAULT_CFG, indent=2), encoding="utf-8")
        print("[O] seeded", f)

def main():
    w(CFX / "alpha" / "feedback.py", CORE)
    w(APP / "ext_alpha_feedback.py", API)
    seed_runtime()
    patch_server()
    print("[O] Bundle O installed.")
